...
resid <- function(data){
  ...
  final_adjusted <- data.frame(cbind(Species=species_list, all_combined)) ### Bind the column "Species" to the final dataset
  write.csv(final_adjusted, "residuals.csv", row.names = FALSE)
